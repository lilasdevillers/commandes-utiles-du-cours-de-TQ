...
test_bw <- svydesign(ids = ~1, data = EE20, weights = ~EE20$EXTRIAN) # configurer l'outil survey sous le nom de test_bw (cela correspond à bw chez la prof)
...
## tests statistiques
res <- svymean(~ACTOP_01, na.rm = TRUE, test_bw) # sauvegarde du résultat de la commande svymean sous le nom de "res"
f.actop <- res[1] # le résultat de la première colonne de svymean (qui donne un tableau) est la fréquence et par cette commande on l'extrait et le nomme "f.actop"
SE.f.actop <- SE(res) # ici on fait qqchose de similaire en extrayant le résultat de la 2e colonne ie la "standard error" SE
stat.student.actop <- (f.actop - 0.402 )/SE.f.actop # calcul de la statistique de test pour ACTOP
df <-length(EE20$ACTOP_01)-1 # calcul du nmbre de d° de liberté
pval <- 2*(1-pt(abs(stat.student.actop),df)) # calcul de la p-value
